...
defineEmits<{
    (e: 'openWindow', href: RoutesNamedLocations): void;
}>();
...
<template>
    <div class="max-w-xs grid grid-cols-2 gap-3 p-2">
        <template v-for="item in items.filter((i) => can(i.permission))" :key="item.title">
            <NuxtLink :to="item.href" class="flex flex-col items-center" @click="$emit('openWindow', item.href)">
                <div class="rounded-lg p-2" :class="[item.iconBackground, item.iconForeground]">
                    <component :is="item.icon" class="w-4" aria-hidden="true" />
                </div>
                <div class="text-neutral" style="text-shadow: 1px rgba(0, 0, 0, 1)">{{ item.title }}</div>
            </NuxtLink>
        </template>
    </div>
</template>
